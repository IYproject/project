			...
			<div class="loginCont">
				...
				<div id="divTabMemberArea" class="yesTab_nor yesTab_blue tab_2col">
					<ul id="ulTabMember">
						...
						<li id="liTabNoMember" class="lastCol"><a href="javascript:void(0);" id="aTabNoMember" onClick="chgTabLogin(this,'nMem');"><span class="txt">예매 확인</span></a></li>
					</ul>
				</div>
				<!-- ############### 탭 영역 끝 ############### -->
				<!-- ############### 로그인 폼 : 회원 영역 시작 ############### -->
...
<form method="post" name="LoginSub" autocomplete="off">
    <input name="FBLoginSub$hdfLoginToken" type="hidden" id="FBLoginSub_hdfLoginToken" value="jj+Q9vDc35+txh6eAqGPaL+VUKju+V1Xv9NWf+5rC8PIod53xEOYsyXlSd0WGywo" />
	...
	<input type="hidden" name="RefererUrl" id="RefererUrl" value="http://www.yes24.com/main/default.aspx?ysmchn=ggl&ysmcpm=google-sponsor&ysmtac=ppc&ysmtrm=yes24&pid=123487&cosemkid=go14913756274066498&gclid=CjwKCAjwjqT5BRAPEiwAJlBuBX2B357NLsXMgbov250_kzyYEGV8HdfVZz1wN3NxF5Wu_10j2Z1Z5xoCsmwQAvD_BwE" />
    ...
    <input name="FBLoginSub$Facebook" type="hidden" id="FBLoginSub_Facebook" value="123487" />
	<fieldset class="loginForm">
		<legend class="blind">회원 로그인</legend>
		<span id="spanMemID" class="yesIpt b_size ipt_wSizeF focus_blue">
			<label for="SMemberID">아이디</label>
			<input id="SMemberID" name="SMemberID" type="text" autocomplete="off" value="" onkeydown="removeErrorText('MemberID')" maxlength="100"/>
		</span>
		...
		<span id="spanMemPW" class="yesIpt b_size ipt_wSizeF focus_blue mgt10">
			<label for="SMemberPassword">비밀번호</label>
			<input id="SMemberPassword" name="SMemberPassword" type="password" autocomplete="off" onkeypress="LoginEnter(event);" onkeydown="removeErrorText('MemberPW')" maxlength="20"/>
		</span>
		...
		<div class="loginForm_chk">
			<span class="yesChk">
				...
				<input id="chkAutoLogin" name="chkAutoLogin" type="checkbox" autocapitalize="off" class="iptChk" />
			</span>
			<span class="yesChk">
				<label for="chkLoginIDSave"><em class="ico_yesChk bgYUI"></em>아이디 저장</label>
				<input id="chkLoginIDSave" name="chkLoginIDSave" type="checkbox" autocapitalize="off" class="iptChk" />
			</span>
		</div>
...
<!-- ##### 아이디 비번 찾기 영역 시작 ##### -->
<div class="loginEtc">
	<a href="/Templates/FTForgetID.aspx" class="lnk_etc">아이디 찾기</a>
	<em class="divi">|</em>
...
<ul class="loginSocialLi">
	...
	<li class="oddCol "><a href="javascript:void(0);" id="FBLoginSub_aBtnFaceBookLogin" onClick="callFaceBookLogin();" class="btn_social"><span class="bWrap"><em class="ico_social ico_fa bgMem"></em><em class="txt">페이스북 아이디로 로그인</em></span></a></li>
	...
	<div class="yesPopUp bgWhite">
		<div class="popYUIArea">
			...
			<div class="popYUI_cont">
                <p class="contP">
                    등록되지 않은 아이디거나, 아이디 또는 비밀번호를 잘못 입력 하셨습니다.<br><br>로그인을 10회 이상 실패하시면 고객님의 정보 보호를 위해 서비스 이용이 차단될 수 있습니다.
                </p>
...
<script type="text/javascript">
    function CheckLogin() {
        ...
        /* //노출제거 요청으로 제거
        if ($naverCode.val() != '' && $naverState.val() != '') {
            $.ajax({
                type: "POST",
                url: '/PartnerSSO/NaverLoginAuth.aspx?type=C',
                data: { 'MEM_ID': $('#SMemberID').val() },
                success: function (res) {
                    if (res == 'False' && confirm('YES24와 네이버의 사용자 이름이 다릅니다.\n연동하시겠습니까?')) {
                        submitLoginSub();
                    } else if (res == 'True') {
                        submitLoginSub();
                    }
    ...
    function callFaceBookLogin() {
        ...
        window.open("https://www.yes24.com/PartnerSSO/FaceBook/FaceBookLoginNew.aspx?ReturnUrl=https://www.yes24.com/PartnerSSO/FaceBook/FaceBookLogin.aspx"
            , "facebook", "toolbar=no, scrollbars=no, width=800, height=600");
    }
...
<script type="text/javascript">
    ...
    function chkFaceBookMem(mid, mnm, email, joinDate) {
        ExistMember(mid, mnm, email, joinDate, '페이스북');
    ...
    function closeloginFailPop() {
        $("#loginFailPop .popYUI_close a").click();
				...
				<div id="nMemLoginForm" class="loginFormGrp" style="display:none;">
					<form name="frmNoMemberLogin" method="post" action="./FTLogin.aspx" id="frmNoMemberLogin" autocomplete="off">
<div>
...
//]]>
						...
						<fieldset class="loginForm">
							<legend class="blind">비회원 예매확인</legend>
							<span id="spanOrdNo" class="yesIpt b_size ipt_wSizeF focus_blue">
								<label for="TxtOrdNo">예매번호</label>
								<input id="TxtOrdNo" name="TxtOrdNo" type="text" autocomplete="off" onkeydown="removeErrorText('OrdNo')" maxlength="25"/>
							</span>
							<p class="yesFormTxt error" style="display:none;" id="pYesFormErrTxt_OrdNo">주문번호를 입력해주세요.</p>
							<!-- <span id="spanOrdConfPwd" class="yesIpt b_size ipt_wSizeF focus_blue mgt10">
								<label for="TxtOrdConfPwd">예매 비밀번호</label>
								<input id="TxtOrdConfPwd" name="TxtOrdConfPwd" type="password" autocomplete="off" onkeypress="LoginEnter2(event);"  onkeydown="removeErrorText('OrdConfPwd')" maxlength="16"/>
							</span>
							<p class="yesFormTxt error" style="display:none;" id="pYesFormErrTxt_OrdConfPwd">주문 비밀번호를 입력해주세요.</p>
							<button type="button" id="btnOrdConf" title="확인" onclick="NoMemOrdConf();return false;" class="btn_login btnC xb_size btn_blue"><span class="bWrap"><em class="txt">확인</em></span></button> -->
						</fieldset>
					</form>
					<!-- ##### 주문번호 찾기 영역 시작 ##### -->
					<div class="loginEtc">
						<a href="./FTForgetOrdNo.aspx" class="lnk_etc">예매번호 찾기</a>
<!-- 						<em class="divi">|</em>
						<a href="./FTForgetOrdPW.aspx" class="lnk_etc">예매 비밀번호 찾기</a>
					</div> -->
					<!-- ##### 주문번호 찾기 영역 끝 ##### -->
				</div>
				<!-- ############### 로그인 폼 : 비회원 영역 끝 ############### -->
